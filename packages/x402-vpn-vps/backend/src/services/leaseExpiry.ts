...
// Redis key patterns (both VPN and VPS are Hetzner servers):
//   server:lease:{serverId}  → JSON { expiresAt: ISO8601 }
...
export async function startLeaseExpiryCron() {
  const INTERVAL_MS = 5 * 60 * 1000; // 5 min — max overrun on a 1-hour lease is 5 min
  setInterval(() => scanExpired().catch(console.error), INTERVAL_MS);
  // Run immediately on startup to catch any servers that expired while offline
  await scanExpired();
